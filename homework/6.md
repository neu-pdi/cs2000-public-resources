...
This HW, like all homeworks, is done on https://github.com and submitted on [Pawtograder](https://pawtograder.khoury.northeastern.edu/). Go to Pawtograder to find the repository where the assignment should be done. The commit and push action automatically makes submissions to Pawtograder, where after a few minutes you will find automated feedback.
...
### Project 2: Interactive Art Installation

Your second (of three) open-ended course projects begins this week. Like other homework, projects will be graded by assignment chats during lab (see [Assignment Chats](/syllabus#assignment-chats)). Since projects are large, each week there will be checkpoints due, which are a part of the project. 

This project gives you the following task:

> Using [reactors](https://pyret.org/docs/latest/reactors.html), [images](https://pyret.org/docs/latest/image.html), and your own creativity, create an interactive experience. It can be educational, tricky, or just plain fun. 

**Project 2, Checkpoint 1**, due this week, is:

- Create a minimal working example of a reactor that uses mouse and keyboard input to change the image that is shown. The images themselves can be simple shapes, or they can be more complex. This week you are focusing on the mechanics, of how reactors work, though as you do so you can obviously start thinking about what your actual installation might be. 
...
All of the below problems will work on a single column (as a list) -- your tests can be standalone, but you are also welcome to try your functions on the table above by extracting the column with `get-column(book-transactions, "column-name")`.
...
Design a function `format-transaction-ids` that takes a list of transaction IDs and converts them to a "display format" by removing the "TXN" prefix and adding "Order #" at the beginning (e.g., "TXN001" becomes "Order #001"). You may assume that the given list only contains transaction IDs. Use `for each` to transform each ID.
