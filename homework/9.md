...
There is no project work this week.
...
Finish the data design for `BinTree` by writing examples for a tree storing numbers and design a template for a function that solves a problem on `BinTree<a>`.

## PART B

### Problem 1

Design a function called `tree-zero` that takes a `BinTree<Number>` and replaces all leaf values with 0; the structure of the tree should remain the same.
...
Design a function called `tree-andmap` that takes a predicate (a function `a -> Boolean`) and a `BinTree<a>` and determines whether all values in the tree satisfy the predicate.

### Problem 3

Design a function called `tree-ormap` that takes a predicate (a function `a -> Boolean`) and a `BinTree<a>` and determines whether any of the values in the tree satisfy the predicate.
