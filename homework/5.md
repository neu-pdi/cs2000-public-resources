...
~~Thursday, February 12th~~ Friday, February 13, 2026 at 9PM (Boston)
...
Reuse your code from Homework 4 to create `housing-table` but make sure that `zipcode` has type `String`. Use it as a model to define a second table called `schools-table` with the appropriate column names, types, and values for the NYC schools data.
...
extracting the closest (i.e., first) row. **NOTE: your function should return the name of the school, not the entire row!**
