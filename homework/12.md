...
You have been provided with a CSV file containing streaming platform viewing data with the following columns: user_id, content_title, genre, watch_time_minutes, rating, subscription_type. Load the CSV file into a DataFrame, named `viewing_data` and complete the following problems.
...
messy_data = {
    ...
    'user_age': [25, 'thirty-five', 28, 32, 72]
}
```

Create a clean version, calling it `clean_data`, that has:
...
Create a copy of your `viewing_data` (from Problem 1), calling it `data_with_hours`:

```
data_with_hours = viewing_data.copy()
```

And then add two new columns to `data_with_hours`:
...
Now create another copy of `viewing_data` (from Problem 1), `data_with_audience`, as:

```
data_with_audience = viewing_data.copy()
```

And add the following columns to `data_with_audience`:
