...
1. Load the CSV from https://pdi.run/f25-2000-photos.csv into a DataFrame. Print the first 5 rows.
...
1. Work with the `durations` Series to find: `.mean()`, `.sum()`, `.max()`, `.min()`.
1. Calculate the range (difference between max and min) of workout durations.
1. For the photos dataset, extract a numeric column and calculate its median using `.median()`.
