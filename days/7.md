...
- Browse and pick a dataset on the [London DataStore](https://data.london.gov.uk/dataset?format=csv). Once you found a dataset to work with, open the page for details on it where you will find (likely more than one) "Download" buttons. **Right-click the "Download" button** for a dataset in CSV format and **copy the link**; do not download it, the Pyret code will do that. Use the copied link (e.g., URL) in your code to create a table from it.
