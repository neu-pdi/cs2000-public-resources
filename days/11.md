...
- For example, the party affiliation, which is often used to target outreach,
  ...
  have typos!
- So if we run `filter-with(voter-data, lam(r :: Row) -> Boolean: get-column(r, "Party") == "Republican" end)`
...
3. Design a function `normalize-phone` that transforms all the phone numbers to
   ...
   digit (e.g., 5551234567). Can you normalize the phone number with just one function call? 
   Use `transform-column` to create a table with these normalized phone numbers.
