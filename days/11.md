...
1. Both State and Zip have a single row that has bad data. Identify what it is.
  Can this be corrected through an automated process? Or can you correct it
  manually?
1. Design a function `normalize-phone` that transforms all the phone numbers to
...
1. Look at the DOB column -- can you normalize these (to, say, YYYY-MM-DD) in the
...
1. Now look at the LastVoted column. Unlike with DOB, there are many rows that do
