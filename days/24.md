...
## 1. Install VSCode
  ...
  **Windows & macOS**
  ...
  2. Your next step will depend on your operating sytem.
     ...
     * On OS X
         * Double-click on the downloaded file to uncompress it.
         * Open a new Finder window with `⌘N`.
         * In the new window, go to the Applications folder through the dropdown menu (`Go > Applications`) or with `⇧⌘A` (shift-command A).
         * Drag `Visual Studio Code 2` from the Downloads folder to the Applications folder.
         * Drag it from the Applications folder to the dock for easy access.
  4. Launch VSCode. 
...
1. Install MyPy Type Checker Extension
  ...
  **Important** Look in the sidebar, perhaps three dots, and check if you see the Python icon:
       ...
       - Windows: `File` > `Preferences` > `Settings` (or `Ctrl` + `,`):
        ...
        Type "Environments Extension" in the search bar. Check the "Python: Use Environments Extension" box. Exit and re-enter VSCode to reload and the python symbol should appear. 
...
You can add Pandas & Pytest to your virtual environment by clicking on the Python icon in the side bar (it may be hidden behind the three dots -- if you don't see this, please review [4.2](#42-check-python-environments-extension) above, and that you installed the Python Environments extension):
...
1. If you get an error about "Github SSO" (seems to occur on Windows), please follow these instructions:
    ...
    a. Download GitHub CLI from cli.github.com and follow the instructions when prompted in the popup.
