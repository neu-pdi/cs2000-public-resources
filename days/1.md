...
- Re-iterate -- for people who do not know how to program. Describe 2100.
...
**Show Definitions vs Interactions**
...
1. Calculate the area of a rectangle with length 10 and width 3.
1. Calculate its perimeter.
1. Note that many different expressions can produce the same value. Come up with
  three different expressions that all **evaluate** to the value 10.
1. Read the beginning of https://pyret.org/docs/latest/numbers.html, on ExactNums
  vs. RoughNums, and then look at some of the functions in the following sections -- unlike
  operators, a function, e.g., `num-max`, when applied to two numbers, is
  written `num-max(10, 20)`. Come up with an expression that uses only one or more ExactNums,
  but **produces** a RoughNum when evaluated.

## Wrap-up (20 mins)

- Share any errors you encountered.
- TASK (IMPORTANT): create an account on github.com (if you don't already have one).
- Go over Skills how this impacts grades on syllabus, how every element of course website ties back to skills. Talk about Assignment Chats, Pair Programming. Talk about AI policy, collaboration policy (the rest they can probably read).
