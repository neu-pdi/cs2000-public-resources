...
1. Add another possibility to `choose-hat`, that if the temperature is below
  ...
  ```
1. Now imagine you got new sunglasses, and want to design a function `add-glasses` that takes
  an outfit (as a string) and always adds ", and glasses" to what you will wear.
1. Let's now design a new function, `choose-outfit`, that takes as input the
...
1. Now, add a new function `choose-hat-or-visor` that takes not only a
