...
- In Pyret, you use `where:` blocks for examples/tests. In Python, there are multiple ways to write tests, but a simple (and widely used) mechanism is using `pytest`, where you write separate test functions (which have to start with `test_`) using `assert`.
...
- **How to run tests:**
  ...
  2. In the VSCode Terminal (Terminal menu -> New Terminal), type:
     ...
     ```
     And hit enter.
     (If this reports that the command is not found, please consult instructions above about installing `pytest`)
