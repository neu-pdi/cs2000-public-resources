...
**NOTE: github.dev has been renamed vscode.dev. It's the same thing, but you may see references to github.dev in videos or notes.**

[Github + VSCode Setup](https://northeastern.hosted.panopto.com/Panopto/Pages/Viewer.aspx?id=a29feff2-4826-4ed6-b5d1-b4bf00da1aff), 
...
[Operations on Images](https://northeastern.hosted.panopto.com/Panopto/Pages/Viewer.aspx?id=991d270c-d86a-4755-a7cf-b4bf00e8f8e7)

## Intro (35 mins)

**Goal** Get started with vscode.dev; learn about algebra on strings & images.

- Mention discord, other course policies that were skipped, as needed.
- Mention first HW out today (due Sunday, Sept 20), ensure they have signed up to https://pawtograder.khoury.northeastern.edu/ -- and linked Github.com account! Docs for Pawtograder: https://docs.pawtograder.com/students/intro
...
- Once it generates, while viewing the repository, type the "." key. This should redirect you to "vscode.dev/github/yourusername/cs2000-scratch".
...
- Then it will ask you to select a github account. Your account should be listed; click on it. These two steps will only happen once (unless you switch computers, use private browsing mode, etc).
...
- Finally click the top icon on the left and click the little icon to make a new file. Type `2.arr` as the file name. This will be your file for today, which is the second class. In general, we'll create a new file for each day within this "scratch" repository. Think of it as a notebook of experiments.
  ![Step 5](/img/5.png)
...
- Add an expression to the definitions window, like "1 + 1", and hit "Enter" (don't hit Run -- that will erase the Interactions on the right and re-run what is in the Definitions window on the left).

  ![Step 6](/img/6.png)


- To save your work, Github requires us to "commit" -- essentially, make a checkpoint with a message describing what we did since the last one. The icon on the left that has circles and lines (and now a blue circle with a 1 in it) gets you to the interface to do so. First, type the message, like "added class 2 file". Now click somewhere below the message, and it lists the "changes". You have to add each change you want to be included in the commit, by clicking the + next to the file. This moves the files from the "Changes" section to the "Staged Changes" section -- these are what will be included in the commit. Once you've added everything, click the checkmark at the top, or the big green button that says "Commit & Push", to make the commit.
  ![Step 7](/img/7.png)

- It's good practice to commit often -- at the very least, every time you do some work that you would rather not lose. If you are working on an assignment for several hours, you should commit many times during that period. While your work may be saved in the interim (your browser does its best to save it), committing _guarantees_ that the work is saved, as it is sent from your browser to a different computer elsewhere in the world (so even if you lost your computer, your work would still exist).
...
- The first is text -- i.e., words, sentences, paragraphs, etc. These are called "strings", and are written as either `"hello cs2000!"` (i.e., double quotes around characters), or if you want the string to be able to cross multiple lines, you can write it as:
...
- We can create basic shapes with functions like `circle`, `rectangle`, `triangle`, e.g.,
  ...
  ```
- Like with numbers and strings, there are operations on images -- e.g., `overlay(i1,i2)` creates a new image that has the first image on top of the second (so if the first image is larger, you will not see the second image at all), vs. `above(i1,i2)` composes the images by creating a new image that includes the two inputs adjacent, one above the other, and `beside(i1,i2)` creates a new image that has the two images provided side by side.

### Class Exercise (15 mins)

1. Look through https://pyret.org/docs/latest/strings.html and find how to turn a string to all uppercase, and apply it to the string "hello cs2000!".
1. Create a **blue circle** and a **yellow rectangle**, then overlay them so the circle appears on top.
1. Stack a green rectangle above a purple rectangle using `above`.
1. Look through https://pyret.org/docs/latest/image.html, and find how to rotate an image; use that to produce a red rectangle that is 100 wide by 20 tall in two different ways.
1. Create a Stop sign (look up an image for reference). `text`, `regular-polygon`, and other functions you've already seen may be helpful.
1. Create another image of your choice!
...
- Further, operations have types that they take in, and types that they produce. These are usually written down in the documentation as contracts, with `::`, e.g.:
  ...
  * :: (Number, Number) -> Number
  ...
  ```
- This captures both the number of arguments (two for `*`, three for `circle`) and the *type* of the arguments (before the "arrow", written `->`) and what it produces (after the arrow), so if you make a mistake in either, Pyret can report that.
