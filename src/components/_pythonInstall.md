...
3. Drag Visual Studio Code from the Applications folder to the dock for easy access
4. Launch VSCode 
...
1. If you get an error about "GitHub SSO" (seems to occur on Windows), please see [these instuctions](#on-windows-i-am-getting-an-error-about-github-sso) under the FAQs
...
4) If all else fails, post in the ``#python-installation`` channel with a screenshot of what is shown in the ``Output`` section of VS Code (click the ``Output`` button next to the ``Terminal`` button) and/or attend office hours for help
...
### I am getting an error when trying to download the extensions
1) On **macOS**, make sure VS Code is in your **Application** folder and not your Downloads folder (you can see this in Finder and can move it to the Application folder). You will have to quit VS Code to move it
...
3) Authorize GitHub, then quit VSCode and reopen it to ensure changes are saved
