...
export default function DaySummary({ version }: { version: string }) {
  ...
  const docs = sidebar.docs
    ...
    .map((doc) => {
      const docContent = require(`@site/days/${doc.id}.md`);
      return {
        doc,
        docContent,
      };
  ...
  docs.sort((a, b) =>
    a.doc.id.localeCompare(b.doc.id, undefined, {
      numeric: true,
      sensitivity: 'accent',
    }),
  );
  return (
    <Box>
      {docs.map(({ doc, docContent }) => {
        const { required_preparation, optional_preparation } =
              ...
              <HStack alignItems="flex-start">
                {required_preparation && (
                  <Box>
                    ...
                    <List.Root m={0}>
                      {required_preparation.map((prep, idx) => (
                        <List.Item key={idx}>
                          <a href={prep}>{prep}</a>
                        </List.Item>
                      ))}
                ...
                {optional_preparation && (
                  <Box>
                    ...
                    <List.Root m={0}>
                      {optional_preparation.map((prep, idx) => (
                        <List.Item key={idx}>
              ...
              <List.Root>
                {headings
                  ...
                  .map((heading, idx) => (
                    ...
                    >
                      <Link to={`${doc.path}#${heading.id}`}>
                        {decode(heading.value)}
                      </Link>
                    </List.Item>
                  ))}
              </List.Root>
