...
- **Present the Template:**
  - Write on the board or display:
    ...
    ```
  - Explain each part:
    - The input list `l` is of type `List<T>` (where `T` is any type).
...
- **Exit Ticket:**
  - Ask each student to write a brief explanation (2–3 sentences) about why the recursive template is so effective for list–processing.
- **Homework Prompt:**
  - Encourage students to write a recursive function (using the template) for a new list–processing problem of their choice (e.g., a function that returns the maximum element of a non-empty list of numbers, handling the base case appropriately).
