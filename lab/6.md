...
For all of these problems, first implement them with `for each`. Afterwards, implement them a second time with `map` or `filter` (_without_ using any mutable variables). For some, it should be possible with just a single call to `L.map` or `L.filter` -- but for others you may need to _also_ use other functions (like `length`).
