...
Create a function called `is-withdrawal` that returns true if a transaction type is "withdrawal". Then, filter with this function to get a list of all withdrawal amounts from the transaction data. Define a constant `TOTAL-WITHDRAWAL` that stores the total amount withdrawn.
