...
Specifically: if the user presses `"b"`, reduce the husky's x-coordinate by `40` pixels (use the `BACK` constant). We need a function that Pyret will call whenever any key is pressed. It returns a new x-coordinate based on which key was pressed:
...
Write the function. Since the output differs based on the input, you'll need a conditional.

### Part C

Add `back-husky` to your reactor (just add the `on-key` line; the rest stays the same):
...
Extend `back-husky` so that `"t"` is a bigger hop left — `4 * BACK` pixels. Think about how to reuse `BACK` rather than writing another magic number.
